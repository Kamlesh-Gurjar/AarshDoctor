  ...
  const handleSubmitForm = async values => {
    ...
    try {
      ...
      const response = await ApiRequest({
        BASEURL: ApiRoutes.addDoctorClinicDetail,
        method: 'POST',
        req: req,
        token: token,
      });
      ...
      console.error('Clinic Add Error:', error?.message || error);
      showErrorToast('Failed', error?.message || 'Error while adding clinic');
    } finally {
      setIsLoading(false);
    }
  };
    ...
    <SafeAreaView style={styles.container}>
      <Formik
        ...
        {({
          ...
          <View style={{flex: 1}}>
            <HeaderCompt title={'Add Clinic Details'} />
            <ScrollView
              ...
              <GooglePlacesAutocomplete
                ...
                //   setFieldValue('state', getComponent('administrative_area_level_1') || '');
                ...
                onPress={(data, details = null) => {
                  ...
                  setFieldValue('address', data.description);
                  setFieldValue('location', data.description); // You might want to remove this if 'address' is sufficient
                  setFieldValue('city', getComponent('locality') || '');
                  setFieldValue(
                    'state',
